...
async function drawBackgroundImage(ctx, canvas) {
  const bgImageUrl = document.body.style.backgroundImage.match(
    /url\(['"]?([^'"]+)['"]?\)/,
  );
  ...
  try {
    ...
    const scale = hasBgImage
      ...
      : (window.devicePixelRatio || 1) *
        SAVE_CONFIG.SCALE_WITHOUT_BG_MULTIPLIER;
    ...
    const html2canvasResult = await html2canvas(
