...
function calculateFinalPositions() {
  ...
  stickers.forEach((sticker) => {
    ...
    if (!isFinite(x) || Math.abs(x) > 10000) {
      logger.warn(`ステッカー${sticker.id} の x座標が異常: ${x} → 0に修正`);
      x = 0;
      sticker.x = 0;
    }
    ...
    if (!isFinite(yPercent) || Math.abs(yPercent) > 200) {
      logger.warn(
        `ステッカー${sticker.id} の yPercent座標が異常: ${yPercent} → 50に修正`,
      );
      yPercent = 50;
      sticker.yPercent = 50;
  ...
  function animate(currentTime) {
    ...
    stickers.forEach((sticker) => {
      ...
      const currentYPercent =
        start.yPercent + (end.yPercent - start.yPercent) * eased;
...
function calculateRepulsionForceVirtual(sticker1, sticker2, positions) {
  ...
  let repulsionMagnitude =
    (LAYOUT_CONFIG.REPULSION_STRENGTH * minDistance) / distance + overlap * 5;
...
function calculateCenterAttractionVirtual(sticker, positions) {
  ...
  const idealRadius =
    (Math.min(window.innerWidth, window.innerHeight) *
      LAYOUT_CONFIG.IDEAL_AREA_RATIO) /
    2;

  // 理想的な範囲を超えている場合のみ引力を働かせる
  if (distance > idealRadius) {
    const excessDistance = distance - idealRadius;
    const attractionMagnitude =
      excessDistance * LAYOUT_CONFIG.CENTER_ATTRACTION;
  ...
  for (const sticker of state.stickers) {
    ...
    if (isOutOfBounds) {
      ...
      const safeX = Math.max(
        -screenWidth * 0.4,
        Math.min(screenWidth * 0.4, sticker.x),
      );
...
async function saveAllStickersWithValidation() {
  ...
  state.stickers.forEach((sticker) => {
    if (!isFinite(sticker.x) || Math.abs(sticker.x) > 10000) {
      logger.error(
        `保存前に異常な x を検出: ステッカー${sticker.id}, x=${sticker.x}`,
      );
      sticker.x = 0;
    }
    ...
    if (!isFinite(sticker.yPercent) || Math.abs(sticker.yPercent) > 200) {
      logger.error(
        `保存前に異常な yPercent を検出: ステッカー${sticker.id}, yPercent=${sticker.yPercent}`,
      );
      sticker.yPercent = 50;
    }
  });
